...
int add(char* employeeName_input, char* supervisorName_input, char* department_input, unsigned int idNumber_input, unsigned int roomNumber_input)
{
	...
	int x = 0;
	//if count is already at max return 2 and don't add employee
	...
	{	//of the input matches anything in the list
		if(list[x].idNumber==idNumber_input)
		{//return 0 and do not add employee to the list
		return 0;
		}
	}
	//Convert 'department_input' to enum and storing in list
	departmentType inputDepartment;
	...
	{
		inputDepartment = Marketing;
	}
	else if (strcmp(department_input, "IT") == 0)
	{
		inputDepartment = IT;
	}
	//adding employee when not duplicate and list not full
	...
	list[count].department = inputDepartment;
	list[count].idNumber = idNumber_input;

	//incrementing count each time something is added to keep track of size
	count++;
	return 1;
}
...
void sort()
{
	...
	const offset = 1;
	//traversal
	for (int z = 0; z < count - offset; z++)
	{
		//inner for loop to find
		for (int y = 0; y < count - z - offset; y++)
		{
			if (list[y].idNumber > list[offset+y].idNumber) // when bigger value id found
			{
				//swapping
				employeeTemp = list[offset + y];
				list[offset + y] = list[y];
				list[y] = employeeTemp;
			}
		}
	}
...
int delete(unsigned int idNumber_input)
{
	struct employeeRecord employeeTemp;	// needed for swapping structs. Not absolutely necessary to use.
	// Write the code below
	int y = 0, z = 0; //initializing
	const int offset = 1;//offset constant

	for (z = 0; z < MAX_EMPLOYEES; z++) //traversal
	{
		if (list[z].idNumber == idNumber_input) break; // if found break

		else if (z == MAX_EMPLOYEES - offset) return 0; // if end of list
	}

	for (y = z; y < count; y++) { //for preparing for deletion
		list[y].roomNumber = list[y + offset].roomNumber;
		list[y].idNumber = list[y + offset].idNumber;
		list[y].department = list[y + offset].department;
		strcpy(list[y].supervisorName, list[y + offset].supervisorName);
		strcpy(list[y].employeeName, list[y + offset].employeeName);	
	}
	//index is deleted, decrementing count per delete, returning value of 1 after successful delete
	count--;
	return 1;
	}
...
void load(char* fileName)
{
	...
	{
		fread(&count, sizeof(count), 1, fileBuffer); // reads tail 
		for (int z = 0; z < count; z++) //traversal
		{
			//individually reading each attribute through the filebuffer
				fread(list[z].employeeName, sizeof(list[z].employeeName), 1, fileBuffer);
				fread(list[z].supervisorName, sizeof(list[z].supervisorName), 1, fileBuffer);
				fread(&list[z].department, sizeof(list[z].department), 1, fileBuffer);
				fread(&list[z].idNumber, sizeof(list[z].idNumber), 1, fileBuffer);
				fread(&list[z].roomNumber, sizeof(list[z].roomNumber), 1, fileBuffer);
		}
