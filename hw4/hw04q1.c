...
void initializeStrings(char strings[NUM_STRINGS][STRING_LENGTH])
{
	...
	int y, z; // the traversal variables
	
	for (y = 0; y < NUM_STRINGS; y++)
	{
		for (z = 0; z < STRING_LENGTH; z++)
		{
			*ptr = '\0'; // filling with \0
			ptr++;
		}
	}
...
void printStrings(char strings[NUM_STRINGS][STRING_LENGTH])
{
	char* ptr = &strings[0][0];
	// Enter the code below
		for(int x = 0; x<NUM_STRINGS; x++) // traversal through every string
		{
			while (*ptr == 0) // while pointer is 0
			{
				ptr++;
			}
			do { printf("%c", *ptr); ptr++; } while (*ptr != 0);
...
char* reverseOneString(char s[STRING_LENGTH])
{
	...
	int len = strlen(s);
	// Enter the code below
	for (int x = 1; x <= len / 2; x++) // traversal, start at 1 to avoid terminator
	{
		temp = *ptr; // set temp equal to pointer
		*ptr = s[len - x]; // swapping
		s[len - x] = temp; // swapping 
		ptr++; // next
	}
	ptr = &s; // storing
	return ptr; // returning newly reversed 
...
void reverseStrings(char strings[NUM_STRINGS][STRING_LENGTH])
{
	// Enter the code below
	char* ptr = &strings[0];
	for(int x = 0; x < NUM_STRINGS; x++) // traversing through every element
	{
		while (*ptr == 0) // while pointer is 0
		{
			ptr++;
		}
		while (*ptr != 0) // while pointer not 0
		{
			ptr++;
		}
		reverseOneString(ptr); // reverse function call per string
	}
}
...
void encryptStrings(char strings[NUM_STRINGS][STRING_LENGTH], int key)
{
	...
	for (int x = 0; x < NUM_STRINGS; x++) // traversal
	{
		while (*ptr == 0) // while it is equal to 0
		{
			ptr++;
		}
		while (*ptr != 0) // while it is not equal to 0
		{
			*ptr = *ptr + key; //encrypting non-0s
			ptr++;
		}
	}
...
void decryptStrings(char strings[NUM_STRINGS][STRING_LENGTH], int key)
{
	...
	// Enter the code below
	reverseStrings(strings);
	for (int x = 0; x < NUM_STRINGS; x++) // traversals
	{
		while (*ptr == 0) // while pointer is 0
		{
			ptr++; // next
		}
		while (*ptr != 0) // when not 0
		{
			*ptr = *ptr - key; // decrypt
			ptr++;
		}
	}
	getchar(); // to flush unintended char
}
...
int isPalindrome(char s[STRING_LENGTH])
{
	...
	// Enter the code below
	int i; // 
	len = strlen(s);// gets string length
	...
	ptr--;
	char* temp = ptr; //set to pointer
	ptr = other; // set pointer to other
	while (*ptr != 0) // while not 0
	{
		if (*ptr != *temp) // if not same
		{
			return 0; // then false
		}	
		ptr++;
	...
	return 1; // returns true if never false!
}
